...
def test_login_UtenteDisabilitato():
    ...
    responseErrMsg = response.json()['detail']
    assert response.status_code == 401
# test sull'utente con username: testUser3
def test_login_OK_PwdScadutaBool():
    '''
    login test - Login corretto, ma l'utenza ha la pwd scaduta
    e quindi riceverà nelle azioni obbligatorie di effettuare il cambio pwd
    '''
    url = ABU_API_BASE_URL + 'login'
    data = {
        'email': 'testUser3', 
    ...
    assert 'CHGPWD' in response_json['requiredActions']
# test sull'utente con username: testUser4
def test_login_OK_PwdScadutaDT():
    '''
    login test - Login corretto, ma l'utenza ha la pwd scaduta
    ...
    assert len(response_json['token']) > 10
    assert 'CHGPWD' in response_json['requiredActions']
# test sull'utente con username: harsh-admin
def test_login_OK():
    '''
    login test - Login corretto, ma l'utenza risulta disabilitata da 
    un utente con privilegi alti
    ...
    assert len(response_json['token']) > 10
    assert len(response_json['requiredActions']) == 0